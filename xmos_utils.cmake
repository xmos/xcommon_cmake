...
function(XMOS_REGISTER_APP)
    ...
    set(APP_OPTIONAL_HEADERS "")
    foreach(lib ${XMOS_TARGETS_LIST})
        get_target_property(inc ${lib} INTERFACE_INCLUDE_DIRECTORIES)
        list(APPEND APP_INCLUDES ${inc})
        get_target_property(opt_hdrs ${lib} OPTIONAL_HEADERS)
        list(APPEND APP_OPTIONAL_HEADERS ${opt_hdrs})
    endforeach()

    list(REMOVE_DUPLICATES APP_SOURCES)
    list(REMOVE_DUPLICATES APP_INCLUDES)
    list(REMOVE_DUPLICATES APP_OPTIONAL_HEADERS)
    list(FILTER APP_OPTIONAL_HEADERS EXCLUDE REGEX "^.+-NOTFOUND$")
    ...
    foreach(inc ${APP_INCLUDES})
        ...
        foreach(header ${headers})
            ...
            list(FIND APP_OPTIONAL_HEADERS ${name} FOUND)
            if(${FOUND} GREATER -1)
